    ...
    def __init__(self, dim_base, dim_cond, use_ref_t, dim_mults=(1, 2, 4)):
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out[1:])):
            self.ups.append(torch.nn.ModuleList([
                     ResnetBlock(dim_out, dim_in, time_emb_dim=dim_base),
...
class Diffusion(BaseModule):
    def __init__(self, n_feats, dim_unet, dim_spk, use_ref_t, beta_min, beta_max):
        ...
        # Initialize the Inceptionx3 module
        self.inception_module = Inceptionx3(in_fts=in_channels, out_fts=out_channels)
...
#            xt_ref += [self.compute_diffused_mean(x_ref, mask, mean_ref, (j+0.5)/15.0)]
        ...
        # Call Inception module on z_estimation and z
        z_estimation_processed = self.inception_module(z_estimation)
        z_processed = self.inception_module(z)
        z_latent = torch.diff(z_processed - z_estimation_processed)
        # Calculate square loss
        dis_loss = torch.mean(torch.square(z_latent))
