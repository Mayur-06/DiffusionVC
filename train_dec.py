...
data_dir = 'Data'
val_file = 'filelists/val_file.txt'
exc_file = 'filelists/exc_file.txt'
...
# Define your device based on the availability of CUDA
...
if __name__ == "__main__":
    ...
    model = DiffVC(n_mels, channels, filters, heads, layers, kernel, 
                   ...
                   dec_dim, beta_min, beta_max).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in tqdm(train_loader, total=len(train_set)//batch_size):
            mel, mel_ref = batch['mel1'].to(device), batch['mel2'].to(device)
            c, mel_lengths = batch['c'].to(device), batch['mel_lengths'].to(device)
            model.zero_grad()
        ...
        with torch.no_grad():
            ...
            for i, (mel, c) in enumerate(mels):
                ...
                mel_lengths = torch.LongTensor([mel.shape[-1]]).to(device)
